...
```shell
docker run -it -v ./data:/data -p 25565:25565  -e EULA=true --name mc_server ghcr.io/andyundso/fabric-server:26.1
```
...
```shell
docker run -e DIFFICULTY=2 -e MOTD="A non-standard message" -e SPAWN_ANIMALS=false ghcr.io/andyundso/fabric-server:26.1
```
